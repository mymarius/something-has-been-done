...
.setDescription(`Cüzdanına rovcoin eklenecek kişiyi etiektlemelisin!`))
  if(!args[1]) return message.channel.send(new Discord.MessageEmbed()
.setColor("RED")
.setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
.setDescription(`⛔ Cüzdana eklenecek rovcoin miktarını girmelisin!`)) 
...
.setColor("GREEN")
.setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
.setDescription(`✅ ${user} kullanıcısının cüzdanına ${money} Rovcoin <a:rovcoin:972225897409155142> eklendi!`))
