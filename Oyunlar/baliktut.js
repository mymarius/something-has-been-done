...
exports.run = async (client, message, args) => {  
   ...
   if (crime !== null && timeout - (Date.now() - crime) > 0) {
     ...
     if(para < 250) return message.channel.send(new Discord.MessageEmbed()
                      .setColor("RED")
                      .setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
                                                .setDescription(`Balık tutmak için balıkçı dayıdan olta kiralamalısın! Gereken rovcoin: 250 Rovcoin <a:rovcoin:972225897409155142>`))
        const result = [
          "WINWIN",
          "LOOSELOOSE"
        ] 
 ...
 var kaybettin = 300         
      message.channel.send(new Discord.MessageEmbed()
                           .setColor("RED")
                      .setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
                      .setDescription(`${cümle} ve ${kaybettin} Rovcoin <a:rovcoin:972225897409155142> kaybettin!`));
 ...
 const sentences2 = ["Çipura tuttun","Hamsi tuttun","Kolyoz tuttun","İsavrit tuttun"]
    ...
    let embed = new Discord.MessageEmbed()
      .setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
      .setColor("GREEN")
      .setDescription(`${sentence2} ve dayı sana ${kazandın} Rovcoin <a:rovcoin:972225897409155142> ödedi!`)
